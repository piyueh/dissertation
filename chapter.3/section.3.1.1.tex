...
\begin{equation}\label{eq:orig-ns-bc}
    ...
    \right.
\end{equation}
We only list the Dirichlet BCs to save space and for the simplicity, as the treatment of Neumann BCs does not differ from treating PDEs in PINNs.
...
In this section we will focus on the solution workflow \cite{dissanayake_neural-network-based_1994,lagaris_artificial_1998,cai_physics-informed_2021} and leave the mathematical expression of neural netwrok models to section \ref{sec:mlp}.
...
\end{equation*}
As the model $G$ is continuous and not defined on discretized space, $x$ can be arbitrary coordinates in the domain.
And $\Delta x$ is a user-provided scalars as there is no grid.
...
This MLP network is said to have one hidden layer with 2 neurons per layer, and it uses cosine for the activation function.
(Note it is uncommon to use cosine for the activation in real applications.)
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
And the same applies to all higher-order derivatives.
...
Even modern hardware and symbolic differentiation software is not able to derive the analytical derivatives when $G$ has tens of thousands of free parameters and when the activation is not as simple as a cosine function.
...
It requires less computing resource than symbolic differentiation and provides more accurate derivatives than numerical differentiation.

Moving on to the next step, once we obtain the desired derivatives, we substitute these derivatives in to the governing PDEs, IC, and BCs.
For example, substituting $G$ and corresponding derivatives to \eqref{eq:orig-ns}, \eqref{eq:orig-ns-ic}, \eqref{eq:orig-ns-bc}, we get several residual functions
\begin{equation}\label{eq:residuals}
    ...
    \end{array}
\end{equation}
We omitted $\vec{x}$, $t$, and $\Theta$ in $G$ to make the expression cleaner. 
...
Also, note that the residuals functions in \eqref{eq:residuals} are continuous in the domain and time range of our interest.
To make the optimization easier, we also limit ourselves to find the minimal residuals at some spatial-temporal coordinates only, rather than asking for minimal residuals everywhere in the continuous space-time domain. 
...
And residuals from different sources are called loss terms.
...
And it is common to see $\alpha_i=1$ in literature.

If there exists zero roosts that make all residuals in \eqref{eq:residual-norms} zero, then a perfect optimization method will find them from either \eqref{eq:hard-constraint-loss}, \eqref{eq:total-residual}, or \eqref{eq:total-residual-weighted}.
This hope is surely unrealistic.
First, zero roots' existence is not guaranteed.
Second, the hyperplane of the aggregated loss, $r(\Theta)$, in the vector space of $\Theta$ is usually neither convex nor concave, making it difficult to find the global minimum.
Even a simple 1D linear Burger's equation with an MLP of around 7,850 free model parameters shows a complicated hyperplane of $r(\Theta)$ \cite{krishnapriyan_characterizing_2021}.
While finding zero roots or the global minimum on the hyperplane of $r(\Theta)$ is difficult, practitioners of PINNs just assume the local minimums of $r(\Theta)$ are good and small enough.
